...
**Status**: **EXECUTED T1–T11** on `feature/agent-config` (11 commits). Automated gate green: `npm run typecheck` + `npm run lint` (0 errors, 4 pre-existing smoke-script warnings) + **183 tests** + `electron-vite build`. **Dev-verified via `scripts/smoke-agent-config.mjs` CDP — 15/15** (Settings agents editor + default-shell persist; New Session ad-hoc chip + input; ad-hoc spawn persists `command` + registry untouched; rename trim/no-op-empty; duplicate independence; concurrency banner ≥4; tinted tile; stopped `lastOutput` preview) + no orphaned shells after cleanup. **Remaining (hand-verify only):** AGCF-05 remove-worktree confirm (needs a registered workspace + worktree) + the visual colour/theme-toggle confirmation; then open PR `Closes #<AM3 issue>`.
