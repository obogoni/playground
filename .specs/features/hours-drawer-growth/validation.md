...
| R3 refusal, pointer absent (HEAD script) | app on a seeded directory, pointer moved aside | exit 1, `not running on the seeded data — no C:\Users\ViniciusSaide\AppData\Local\Temp\playground-smoke-hours.last; run with --seed first`. Probe before and after: `{"sessions":0,"periods":14,"open":0,"seed":14}` |
