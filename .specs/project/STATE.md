...
**Current Work:** **force-remove-worktree — EXECUTING** on `feature/force-remove-worktree` (branched off updated `main`, which now carries agent-task-context PR #47 + worktree-base-refresh PR #52). New post-v2 enhancement: opens a deliberate, informed force-remove path for **dirty** worktrees (the v1 _Delete Worktree (guarded)_ feature deliberately left these UI-unremovable). Clicking Remove on a dirty worktree opens a confirmation that **lists the changed files (path + status label: Modified/Added/Deleted/Untracked/Renamed)** fetched fresh, then `git worktree remove --force` on confirm. Primary-checkout refusal untouched (git can't force-remove a main working tree). **Backend already supports `{ force }`** (tested) — the new backend work is only a porcelain→`ChangedFile[]` reader (`parseChangedFiles`/`changedFilesOf`). **User decisions (3 gray areas resolved via discuss):** (1) detail = path + human status label; (2) friction = single red "Discard & remove" button, no type-to-confirm; (3) dirty button arms as enabled danger (not disabled-look), note flips to "N changes will be discarded on remove". **Plan:** 7 tasks (`.specs/features/force-remove-worktree/{spec,tasks}.md`, FRWT-01..04, design inline): T1 shared types + `force` in IPC + `worktrees:changes` channel · T2 backend parser/reader · T3 main wiring (drop the `index.ts:106` "no force path" comment) · T4 extend `RemoveWorktreeConfirm` (changes section + adaptive copy, drop the stale "can't be removed" note) · T5 `WorktreeDetail` (arm dirty button, fetch changes, `force: dirty` on confirm) · T6 unit tests (parser + force-remove on temp repos) · T7 extend `smoke-remove.mjs` + full gate. Dirty+agents = one merged dialog. **Note:** baseline test count moved 183→190 (worktree-base-refresh +7); confirm fresh `N` with `npm test`. (Prior: worktree-base-refresh.)
...
- [ ] **Execute force-remove-worktree (T1–T7)** on `feature/force-remove-worktree` — shared types + `force` IPC + `worktrees:changes` · `parseChangedFiles`/`changedFilesOf` · main wiring · `RemoveWorktreeConfirm` changes section · armed dirty button in `WorktreeDetail` · unit tests · smoke + gate. Then open PR (`Closes #54`)
