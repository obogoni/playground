/**
 * WF1 spike wiring (disposable — proves the WF3 `mcp-result-server` shape).
 *
 * A single loopback HTTP MCP server (`@modelcontextprotocol/sdk`) that forces
 * structured output out of the headless agent (Arm M). One bearer token per
 * step BOTH routes and authorizes: a request with a missing/unknown/revoked
 * token is rejected before it reaches any MCP transport (WF1-03). For a
 * registered token the server exposes a single `emit_result` tool whose
 * `inputSchema` is built from that step's `expect` via the T2 pure seam, and a
 * valid `emit_result` call resolves the pending promise with the validated
 * payload (WF1-04).
 *
 * Low-level `Server` + `setRequestHandler` (not the high-level `McpServer`) is
 * used deliberately: the spike's contract is that `tools/list` returns EXACTLY
 * `buildToolInputSchema(expect)`, and only the low-level path emits a raw JSON
 * Schema verbatim — `registerTool` would round-trip it through Zod and reshape
 * it.
 */
...
function buildTokenServer(reg: Registration): Server {
  const server = new Server(
    { name: 'wf1-result-server', version: '0.0.0' },
    { capabilities: { tools: {} } }
  )
    ...
    tools: [
      {
        ...
        // Verbatim from the pure seam — this is the exact contract WF1-04 asserts.
  ...
  server.setRequestHandler(CallToolRequestSchema, (request) => {
    ...
    if (!result.ok) {
      // The spike only REPORTS the mismatch; the corrective --resume retry is WF3.
      return {
